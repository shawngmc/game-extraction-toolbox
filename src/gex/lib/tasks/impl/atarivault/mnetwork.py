...
def find_files(base_path):
    ...
    uncomp_rom_path = os.path.join(base_path, "AtariVault_Data", "StreamingAssets", "FOCAL_Emulator", "vol3")
...
def copy(in_dir, out_dir):
    ...
    for file_path in rom_files:
        ...
        if game_info is not None:
            ...
            logger.info(f"Copying {file_name} to {game_info['filename']}: {display_name}")
            try:
